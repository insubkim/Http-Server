...
HttpResponse::HttpResponse(const HttpRequest& req, const RouteRule& route_rule) : 
  _http_major(1),     _http_minor(1),     _status(0), \
  _content_length(0), _is_chunked(false), _is_ready(false), \
  _is_cgi(false),     _cgi_handler(req, route_rule), \
  _entity_idx(0),     _header_idx(0),     _is_header_sent(false), \
  _is_session_block(false), _is_logout_req(false), \
  _eof(false), _request_method(HPS::kHEAD) 
  {
