...
class HttpResponse {
  ...
  public:
    ...
    void                                      initializeCgiProcess(const HttpRequest& req, const RouteRule& rule, \
                                                  const std::string& server_name, const int& port, const int& client_fd);
    int                                       cgiExecute(const std::map<std::string, SessionBlock>::const_iterator& sbi, bool is_joined_session);
